...
def add_custom_metadata(input_path, output_path):
    ...
    cmd = [
        ...
        "-c", "copy",
        "-map_metadata", "0",
        "-metadata", "title=Test Video - Test Channel",
        "-metadata", "VIDEO_ID=test123",
        "-metadata", "SOURCE=youtube",
        "-metadata", "PLAYLIST_ID=PL_test",
        "-metadata", "PURL=https://www.youtube.com/watch?v=test123",
        "-metadata", "artist=Test Channel",
        str(output_path)
    ]
    success, error = run_ffmpeg(cmd)
    if not success:
...
def simulate_subtitle_embed(input_path, output_path):
    ...
    cmd = [
        ...
        "-map", "0:v",
        "-map", "0:a",
        "-map_metadata", "0",  # This is critical!
        "-c:v", "copy",
        "-c:a", "copy",
        str(output_path)
    ]
